...
public class ProductValidator implements Validator<Product> {
	...
	public ValidationResult validate(final Product product) {
		...
		if (category == null) {
			support.addError(JsamsI18nLabelResource.LABEL_PRODUCT_CATEGORY
		...
		if (price == null || ValidationUtils.isBlank(price.toPlainString())) {
			support.addError(JsamsI18nLabelResource.LABEL_PRODUCT_PRICE
					.getTranslation(), JsamsI18nResource.ERROR_IS_MANDATORY
					.getTranslation());
		}
		int quantityStock = product.getQuantityStock();
		if (quantityStock == 0 || ValidationUtils.isBlank(Integer.toString(quantityStock))) {
			support.addError(JsamsI18nLabelResource.LABEL_PRODUCT_STOCK_QUANTITY
					.getTranslation(), JsamsI18nResource.ERROR_IS_MANDATORY
					.getTranslation());
		}
