...
public class SocietyBeanView extends AbstractView<SocietyBean, JPanel, JPanel> {
    ...
    public JPanel createEditView() {
        ...
        JTextField textFieldName = helper.createBindingTextComponent(bean, SocietyBean.NAME_PROPERTY, true, false);
        JFormattedTextField textFieldCapital = helper.createBindingDecimalComponent(bean, SocietyBean.CAPITAL_PROPERTY,
                true, false);
        JTextField textFieldActivity = helper.createBindingTextComponent(bean, SocietyBean.ACTIVITY_PROPERTY, true,
                false);
        JTextField textFieldResponsible = helper.createBindingTextComponent(bean, SocietyBean.RESPONSIBLE_PROPERTY,
                false, false);
        JTextField textFieldVatNumber = helper.createBindingTextComponent(bean, SocietyBean.VATNUMBER_PROPERTY, false,
                false);
        ...
        builder.appendI15d(JsamsI18nLabelResource.LABEL_LEGAL_FORM.getKey(),
                bean.getLegalForm().getView().createEditView(), 1);
        builder.nextLine();
